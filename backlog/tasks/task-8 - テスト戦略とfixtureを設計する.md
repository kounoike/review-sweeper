...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 単体、ドメイン、API、fixture、統合、WindowsネイティブUIのテスト境界を定義する
- [ ] #2 GitHub差分、レビューコメント、Checks、各種worktree状態、実行バックエンドのfixture方針を決める
- [ ] #3 ネットワーク、GitHubアカウント、WSL導入に依存しないCIテスト方法を決める
- [ ] #4 非同期処理、キャンセル、再試行、破損状態、backend混在防止を検証する項目を定義する
<!-- AC:END -->
